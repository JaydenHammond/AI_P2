## Problema de Satisfacción de Restricciones es una búsqueda informada que busca asignar valores a variables de manera que se cumplan todas las restricciones dadas.
## f(n) = h(n), donde h es la evaluación de la satisfacción de las restricciones; se busca una solución que cumpla todas las restricciones del problema.
...
def satisfaccion_restricciones(tareas, alumnos, habilidades, restricciones):
    asignacion = {}
...
asignacion_tareas = satisfaccion_restricciones(tareas, alumnos, habilidades, restricciones)
